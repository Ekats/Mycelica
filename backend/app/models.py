...
class ClusterName(Base):
    ...
    level = Column(String, nullable=False)  # 'galaxy', 'cluster', 'topic', 'message'
    name = Column(String, nullable=False)
    keywords = Column(Text)  # JSON array of keywords
    created_at = Column(DateTime, nullable=False, default=datetime.utcnow)
    is_manual = Column(Integer, default=1)  # 1 for manual, 0 for AI-generated


class TopicTags(Base):
    __tablename__ = "topic_tags"

    topic_id = Column(String, primary_key=True)  # topic_gX_cY_tZ or conversation_id
    tags = Column(Text, nullable=False)  # JSON array of tags
    created_at = Column(DateTime, nullable=False, default=datetime.utcnow)
    is_ai_generated = Column(Integer, default=1)  # 1 for AI, 0 for manual
